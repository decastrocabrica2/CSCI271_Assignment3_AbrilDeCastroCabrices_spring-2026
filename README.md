...
it ends up with a new string each call, so the complexity is O(n^2)
...
the Math.max method. Therefore the time complexity is O(n)
...
so it doesnt add unwanted numbers to our sum, furthermore the complexity is O(n). 
